...
import { DEFAULT_PITCH, MarbleController } from "./marble_controller";

export class LocalMarbleController extends MarbleController {
	yaw = 0;
	pitch = DEFAULT_PITCH;
	jumping = false;
	using = false;
	...
	applyControlState() {
		...
		if (state) this.marble.currentControlState = state;
	}
